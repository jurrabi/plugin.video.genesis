...
#JLU            content = '%s?action=play&name=%s&title=%s&year=%s&imdb=%s&tmdb=%s' % (sys.argv[0], sysname, systitle, year, imdb, tmdb)
# Use exodus to play instead of genesis that doesn't always work and is not being updated anymore.
            content = '%s?action=play&name=%s&title=%s&year=%s&imdb=%s&tmdb=%s' % ('plugin://plugin.video.exodus/', sysname, systitle, year, imdb, tmdb)
...
class libtvshows:
    ...
    def strmFile(self, i):
        try:
            ...
            content = '%s?action=play&name=%s&title=%s&year=%s&imdb=%s&tmdb=%s&tvdb=%s&tvrage=%s&season=%s&episode=%s&tvshowtitle=%s&alter=%s&date=%s' % ('plugin://plugin.video.exodus/', episodename, episodetitle, year, imdb, tmdb, tvdb, tvrage, season, episode, systitle, alter, syspremiered)
        ...
        try:
            ...
            for file in episode:
                try:
                    ...
                    if not read.startswith('plugin://plugin.video.exodus/'): raise Exception()
